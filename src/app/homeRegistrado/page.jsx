...
// ============= MOCK DATA (sustituir por BD cuando esté lista) =============
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 h-16 transition-all duration-300 ${
        ...
        <ul className="hidden lg:flex items-center gap-8 list-none">
          {["Inicio", "Explorar", "Mi Biblioteca", "Amigos", "Listas"].map((link) => (
            <li key={link}>
              <a href="#" className="text-sm font-medium text-foreground/60 hover:text-foreground transition-colors duration-200">{link}</a>
            </li>
          ))}
        </ul>
        <div className="flex items-center gap-3">
        ...
        <div className="relative z-20 text-center px-4 max-w-3xl mx-auto flex flex-col items-center gap-6">
          <div className="relative">
            ...
            <div className="relative bg-background/30 backdrop-blur-xl rounded-3xl p-6 border border-foreground border-opacity-30">
              <Image src="/logotipo.png" alt="CHECKPOINT" width={120} height={120} priority className="drop-shadow-2xl" />
            </div>
          </div>
          <div className="space-y-2">
            <p className="text-xs font-semibold text-foreground/50 tracking-[0.2em] uppercase">Tu diario de videojuegos</p>
            <h1 className="text-6xl font-black text-foreground tracking-widest drop-shadow-lg">CHECKPOINT</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-3">
              {MY_LIBRARY.map((game) => (
                <div key={game.id} className="group relative bg-foreground/5 border border-foreground/10 rounded-2xl overflow-hidden hover:border-foreground/25 hover:-translate-y-1 transition-all duration-200 cursor-pointer">
                  <div className="relative h-24 overflow-hidden">
                    <Image src={game.cover} alt={game.title} fill sizes="(max-width: 1024px) 50vw, 25vw" className="object-cover brightness-50 group-hover:brightness-60 transition-all duration-300" />
                  ...
                  <div className="p-3">
                    ...
                    <div className="h-1 bg-foreground/10 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full transition-all duration-500 ${STATUS_BAR_COLORS[game.status]}`} style={{ width: `${game.progress}%` }} />
                    </div>
