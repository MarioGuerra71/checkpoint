...
export default function MisAmigosPage() {
  ...
  const dejarDeSeguir = async (id_usuario) => {
    try {
      await fetch(`/api/seguimiento?usuarioId=${id_usuario}`, { method: "DELETE" });
      setSeguimiento(prev => ({
        ...prev,
  ...
  const lista = activeTab === "siguiendo"
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <nav className="flex items-center justify-between px-8 h-16 border-b border-foreground/10 bg-background/80 backdrop-blur-xl sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/buscar" className="text-sm font-bold text-background bg-foreground px-4 py-1.5 rounded-lg hover:brightness-90 transition-all cursor-pointer">
            Buscar usuarios
          </Link>
        </div>
      </nav>
      ...
      <main className="max-w-3xl mx-auto px-6 py-12 space-y-8">
        ...
        <h1 className="text-3xl font-black text-foreground tracking-widest uppercase">Mis Amigos</h1>
        ...
        {loading || loadingUser ? (
          ...
          <div className="text-center py-16">
            <p className="text-5xl mb-4">{activeTab === "siguiendo" ? "🔭" : "👥"}</p>
            ...
            {activeTab === "siguiendo" && (
              <Link href="/buscar" className="mt-4 inline-block px-6 py-2.5 rounded-xl font-bold text-background bg-foreground text-sm hover:brightness-90 transition-all">
                Buscar usuarios
              </Link>
            )}
          </div>
