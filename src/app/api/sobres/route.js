...
function sortearRareza() {
  ...
  if (rand < PROBABILIDADES.legendario + PROBABILIDADES.epico + PROBABILIDADES.raro) return "raro";
  return "comun";
}
...
export async function POST(req) {
  try {
    ...
    if (!sobre) return NextResponse.json({ error: "Error de usuario" }, { status: 400 });

    if (tipo === "diario") {
      if (sobre.ultimo_sobre) {
        const diffHoras = (new Date() - new Date(sobre.ultimo_sobre)) / (1000 * 60 * 60);
        if (diffHoras < 24) {
          return NextResponse.json(
      ...
      if (sobre.sobres_pendientes <= 0) {
        return NextResponse.json({ error: "No tienes sobres pendientes" }, { status: 400 });
      }
    }
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (duplicado) {
        ...
        resultados.push({ ...item, duplicado: true, monedasRecibidas: monedas });
      } else {
