...
export async function GET(req) {
  try {
    ...
    const [resenas] = await db.query(
      `SELECT r.id_resena, r.puntuacion, r.comentario, r.plataforma, r.fecha_resena,
...
export async function POST(req) {
  try {
    ...
    const { rawg_game_id, puntuacion, comentario, plataforma } = await req.json();
    ...
    await db.query(
      `INSERT INTO resena (rawg_game_id, id_usuario, puntuacion, comentario, plataforma, fecha_resena)
       VALUES (?, ?, ?, ?, ?, NOW())`,
      [rawg_game_id, id_usuario, puntuacion || null, comentario?.trim() || null, plataforma || null]
    );
...
export async function PUT(req) {
  try {
    ...
    const { id_resena, puntuacion, comentario, plataforma } = await req.json();
    ...
    await db.query(
      `UPDATE resena SET puntuacion = ?, comentario = ?, plataforma = ? WHERE id_resena = ?`,
      [puntuacion || null, comentario?.trim() || null, plataforma || null, id_resena]
    );
