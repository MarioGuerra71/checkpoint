...
export async function GET(req) {
  try {
    ...
    if (!listaId) return NextResponse.json({ error: "listaId requerido" }, { status: 400 });
...
export async function POST(req) {
  try {
    ...
    if (!id_lista || !rawg_game_id) {
      return NextResponse.json({ error: "id_lista y rawg_game_id requeridos" }, { status: 400 });
    }
    ...
    if (existe.length > 0) {
      return NextResponse.json({ error: "El juego ya está en esta lista" }, { status: 409 });
    }
...
export async function DELETE(req) {
  try {
    ...
    if (!listaId || !gameId) {
      return NextResponse.json({ error: "listaId y gameId requeridos" }, { status: 400 });
    }
