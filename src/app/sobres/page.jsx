...
function ItemCard({ item, equipado, onEquipar, compacto = false }) {
  ...
  return (
    <div
      className={`relative border-2 ${cfg.color} ${cfg.bg} rounded-2xl p-3 flex flex-col items-center gap-2 transition-all duration-200 hover:-translate-y-1 ${item.duplicado ? "opacity-60" : ""} ${equipado ? "ring-2 ring-foreground/40" : ""}`}
    >
      {/* Badge rareza */}
      <span
        className={`absolute top-2 right-2 text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-full ${cfg.bg} ${cfg.texto} border ${cfg.color}`}
      >
        {cfg.label}
      </span>
      ...
      {/* Imagen */}
      <div
        className={`relative ${compacto ? "w-12 h-12" : "w-16 h-16"} rounded-xl overflow-hidden bg-background/50 flex items-center justify-center`}
      >
        <Image
          src={item.imagen_url}
          alt={item.nombre}
      ...
      <p
        className={`${compacto ? "text-[10px]" : "text-xs"} font-bold text-foreground text-center leading-tight`}
      >
        {item.nombre}
      </p>
    ...
    <div className="fixed inset-0 z-400 flex items-center justify-center bg-black/90 backdrop-blur-md">
      <div className="text-center space-y-8 px-4 max-w-lg w-full">
        ...
        {(fase === "cerrado" || fase === "abriendo") && (
          <div
            className={`mx-auto transition-all duration-700 ${fase === "abriendo" ? "scale-125 opacity-0" : "scale-100 opacity-100"}`}
          >
            ...
            <p className="text-foreground/60 text-sm mt-4 animate-pulse">
              Abriendo sobre...
            </p>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              ...
              {Array.from({ length: 3 - itemsMostrados.length }).map((_, i) => (
                <div
                  key={`placeholder-${i}`}
                  className="w-24 h-36 rounded-2xl bg-foreground/5 border-2 border-foreground/10 animate-pulse"
                />
              ))}
            </div>
          </div>
...
export default function SobresPage() {
  ...
  useEffect(() => {
    ...
    const objetivo = new Date(
      Date.now() + estadoSobre.horasRestantes * 60 * 60 * 1000,
    );
            ...
            {activeTab === "sobres" && estadoSobre && (
              <div className="space-y-8">
                ...
                <div className="bg-foreground/5 border border-foreground/10 rounded-2xl p-8 text-center space-y-4">
                  ...
                  <div
                    className={`w-32 h-40 mx-auto rounded-2xl border-2 flex items-center justify-center transition-all duration-300 ${
                ...
                {estadoSobre.sobresPendientes > 0 && (
                  <div className="bg-foreground/5 border border-foreground/10 rounded-2xl p-6 text-center space-y-4">
                    ...
                    <p className="text-xs text-foreground/40">
                      Sobres de bienvenida sin abrir
                    </p>
                    <button
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {Object.entries(RAREZA_CONFIG).map(([rareza, cfg]) => (
                    <div
                      key={rareza}
                      className={`${cfg.bg} border ${cfg.color} rounded-xl p-3 text-center`}
                    >
                      <p className={`text-sm font-black ${cfg.texto}`}>
                        {cfg.label}
                      </p>
              ...
              <div className="space-y-8">
                {inventario.length === 0 ? (
                  ...
                  <>
                    {avatares.length > 0 && (
                      <div>
                        <h3 className="text-sm font-black text-foreground/50 tracking-widest uppercase mb-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {tienda.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id_item}
                          className={`border-2 ${cfg.color} ${cfg.bg} rounded-2xl p-4 flex flex-col items-center gap-3`}
                        >
                          <span
                            className={`text-[10px] font-black uppercase ${cfg.texto}`}
                          >
                            {cfg.label}
                          </span>
